...
def update_s3_file(dataframe, s3_key):
    ...
    try:
        ...
        s3.load_string(string_data=csv_buffer.getvalue(),
            key=s3_key,
            bucket_name=Variable.get("s3_bucket_name"),
            replace=True
        )
...
def s3_to_300movie_list():
    try:
        s3 = S3Hook(aws_conn_id='aws_conn')
        obj = s3.get_key(key="naver/알바영화리스트.csv", bucket_name=Variable.get('s3_bucket_name'))
        if obj:
            csv_data = obj.get()['Body'].read().decode('utf-8')
    ...
    for movie in movies:
        ...
        try:    
            ...
            for review in reviews:
                ...
                if review_content != " ":
                    review_date = review.get_attribute('data-report-time')[:8]  # 작성 날짜 
                    review_time = review.get_attribute("data-report-time")[-5:] # 작성 시간
                    review_id = review.get_attribute("data-report-writer-id")   # 작성 아이디
                    review_score = review.find_element(By.CLASS_NAME, "area_text_box").text[12:].replace("\n","")
...
with DAG(
    ...
    s3_to_300movie_list = PythonOperator(
        task_id = 's3_to_300movie_list',
        python_callable=s3_to_300movie_list,
        dag=dag
    )
